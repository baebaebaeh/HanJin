...
-- 아래 줄을 실행해보면 40번의 부서에는 사람이 없다는것을 알 수 있다.
...
SELECT COUNT(*) 사원수, SUM(sal) 합, AVG(sal) 평균, MAX(sal), MIN(sal)
FROM emp;

# Q23. 모든 사원에 대하여 부서, 사원수, 급여총액, 평균급여, 최고급여, 최저급여를 부서별로 조회하고, 소수점 둘째자리 반올림
SELECT deptno 부서번호, COUNT(*) 사원수, ROUND(AVG(sal), 2), MAX(sal), MIN(sal)
FROM emp
GROUP BY deptno;
